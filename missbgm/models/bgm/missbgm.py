"""Standalone MissBGM model for MNAR imputation."""
...
class MissBGM(BGM):
    """Bayesian generative model specialized for MNAR imputation."""
    ...
    @tf.function
    def _update_theta(self, data_z: tf.Tensor, data_x: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
        with tf.GradientTape() as tape:
    ...
    @tf.function
    def _update_phi(self, data_x: tf.Tensor, mask: tf.Tensor) -> tf.Tensor:
        with tf.GradientTape() as tape:
    ...
    def _evaluate_state(
        ...
        if x_true_original is not None and inverse_transform is not None:
            x_full_original = inverse_transform(x_full)
            metrics["rmse_missing_only_original_space"] = rmse_on_missing_entries(x_true_original, x_full_original, mask)
        return metrics
    ...
    def fit(
        ...
        for epoch in range(1, self.params["epochs"] + 1):
            ...
            epoch_metrics = {"latent_map_loss": [], "x_map_loss": [], "theta_loss": [], "phi_loss": []}
            ...
            for start in range(0, n_rows, batch_size):
                ...
                for _ in range(self.params["n_inner_steps"]):
                    latent_map_loss, _, _ = self._map_update_z(self.data_z, self.data_x, batch_indices, self.z_optimizer)
            ...
            if epoch % self.params["epochs_per_eval"] == 0 or epoch == self.params["epochs"]:
                metrics = self._evaluate_state(
                    x_obs,
                    resolved_mask,
                    self.data_z,
                    self.data_x,
                    x_true=x_true,
                    x_true_original=x_true_original,
    ...
    def _run_map_inference(
        ...
        for epoch in range(epochs):
            ...
            for start in range(0, n_rows, batch_size):
                ...
                for _ in range(self.params["n_inner_steps"]):
                    latent_map_loss, _, _ = self._map_update_z(data_z, data_x, batch_indices, z_optimizer)
                    epoch_metrics["latent_map_loss"].append(float(latent_map_loss.numpy()))
            ...
            if epoch % self.params["epochs_per_eval"] == 0:
                ...
                if verbose:
                    print(
                        "{} Epoch [{}/{}] latent_map={:.4f} x_map={:.4f} rmse={}".format(
                            "Adapt" if adapt else "Test",
                            epoch,
                            epochs,
                            metrics["latent_map_loss_mean"],
                            metrics["x_map_loss_mean"],
    ...
    def _assert_finite_numpy(self, array: np.ndarray, name: str) -> None:
        if not np.isfinite(array).all():
            raise FloatingPointError(f"{name} became non-finite during MissBGM inference.")

    @staticmethod
    def _clip_gradient(gradient: tf.Tensor, clip_norm: float, batch_mask: Optional[tf.Tensor] = None):
        if gradient is None:
            raise FloatingPointError("Received a None gradient during MissBGM posterior updates.")
    ...
    def _posterior_outputs_from_state(
        ...
        x_full_for_z = tf.Variable(reconstruct_from_mask(x_obs_tf, mask_tf, x_state), trainable=False, dtype=tf.float32)
        ...
        for step in range(total_steps):
            ...
            z_kernel_results = refresh_conditional_results(z_kernel_results, z_state, z_target_log_prob_fn)
            z_state, z_kernel_results = z_kernel.one_step(
                z_state,
                z_kernel_results,
                seed=None if seed is None else seed + 2 * step,
            )
            z_acceptance.append(float(tf.reduce_mean(tf.cast(z_kernel_results.inner_results.is_accepted, tf.float32)).numpy()))

            if has_missing:
                z_for_x.assign(z_state)
                x_kernel_results = refresh_conditional_results(x_kernel_results, x_state, x_target_log_prob_fn)
                x_state, x_kernel_results = x_kernel.one_step(
                    x_state,
                    x_kernel_results,
                    seed=None if seed is None else seed + 2 * step + 1,
                )
                x_acceptance.append(float(tf.reduce_mean(tf.cast(x_kernel_results.inner_results.is_accepted, tf.float32)).numpy()))
        ...
        x_imputed_mcmc_mean = np.mean(sample_predictions, axis=0).astype(np.float32)
        ...
        outputs = {
            ...
            "mcmc_mean_imputed": x_imputed_mcmc_mean,
