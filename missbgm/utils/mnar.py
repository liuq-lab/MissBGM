...
def validate_mask(mask: np.ndarray, expected_shape: Optional[Tuple[int, int]] = None) -> np.ndarray:
    ...
    if expected_shape is not None and mask_array.shape != expected_shape:
        raise ValueError(f"Mask shape {mask_array.shape} does not match expected shape {expected_shape}.")
...
def infer_mask(data: np.ndarray) -> np.ndarray:
    return (~np.isnan(_to_float32_matrix(data))).astype(np.float32)
...
def prepare_masked_data(
    ...
    resolved_mask = infer_mask(array) if mask is None else validate_mask(mask, expected_shape=array.shape)
...
def reconstruct_from_mask(x_obs, mask, x_mis):
    ...
    if x_mis_array.shape != x_obs_array.shape:
        raise ValueError(f"x_mis shape {x_mis_array.shape} does not match x_obs shape {x_obs_array.shape}.")
...
class ObservedStandardizer:
    ...
    def fit(self, x_obs: np.ndarray, mask: np.ndarray) -> "ObservedStandardizer":
        ...
        self.mean_ = np.where(counts > 0.0, mean, 0.0).astype(np.float32)
        self.scale_ = np.where(counts > 0.0, scale, 1.0).astype(np.float32)
        return self
    ...
    def to_dict(self) -> dict:
        self._check_is_fitted()
        return {"mean": self.mean_.tolist(), "scale": self.scale_.tolist(), "min_scale": float(self.min_scale)}
...
def prediction_intervals_from_samples(samples: np.ndarray, mask: np.ndarray, alpha: float = 0.05) -> Sequence[np.ndarray]:
...
class MissForestRegressorImputer:
    def __init__(self, n_estimators: int = 100, max_iter: int = 5, random_state: Optional[int] = 123) -> None:
...
def knn_imputation_baseline(x_obs: np.ndarray, mask: np.ndarray, n_neighbors: int = 5) -> np.ndarray:
    observed, resolved_mask = prepare_masked_data(x_obs, mask)
...
def missforest_imputation_baseline(
    ...
    return MissForestRegressorImputer(
        n_estimators=n_estimators,
        max_iter=max_iter,
        random_state=random_state,
    ).fit_transform(x_obs, mask)
...
def benchmark_mnar_imputers(
    ...
    for method in tuple(str(method) for method in methods):
        if method == "mean":
            x_imputed = mean_imputation_baseline(x_obs, mask)
        elif method == "knn":
            x_imputed = knn_imputation_baseline(x_obs, mask, n_neighbors=int(params.get("knn_neighbors", 5)))
